# Jigsaw Product Architecture Specification
...
Jigsaw is feedback infrastructure for modern applications. It gives developers a simple SDK to collect contextual, lightweight, standardized user feedback inside their apps, then routes that feedback into a dashboard, analytics layer, integrations, and product workflows.
...
> Every app needs feedback, but most teams collect it through scattered emails, support tickets, Discord messages, surveys, hacked-together buttons, and random user complaints. Jigsaw standardizes feedback collection by letting apps ask the right question at the right moment and convert responses into structured product signal.

Naming note:

- Customer-facing product/company name: **Jigsaw**
- Current domain: **jigsawkit.com**
- SDK/package namespace for now: **`@jigsawkit/*`**
- Runtime client object in examples: **`jigsaw`**

Use **Jigsaw** in product copy, dashboard UI, docs, and marketing. Use **JigsawKit** only when referring to the current domain or package/distribution namespace.
...
Jigsaw should ask for feedback only when the user just experienced something meaningful.
...
This section gives the coding session a visual map of how Jigsaw should work across the frontend, backend, SDK, database, and integrations. These diagrams use Mermaid syntax so they can render directly in GitHub, many Markdown editors, and some documentation tools.
...
flowchart LR
  subgraph CustomerApp[Customer Application]
    AppUI[App UI]
    SDK[Jigsaw Browser SDK]
    ReactSDK[Jigsaw React SDK]
  ...
  subgraph Dashboard[Jigsaw Dashboard]
...
sequenceDiagram
  ...
  participant SDK as Jigsaw SDK
  participant API as Jigsaw Public API
...
flowchart TD
  ...
  D --> E[Installs @jigsaw/browser or @jigsaw/react]
...
```mermaid
flowchart TD
  Dashboard[Jigsaw Dashboard] --> Overview[Overview]
...
Contains a demo application showing how Jigsaw works.
...
A customer application using Jigsaw.
...
```txt
x-jigsaw-project-key: pk_live_abc123
...
```ts
jigsaw.init(options)
jigsaw.identify(userId, traits)
jigsaw.track(eventName, metadata)
jigsaw.ask(options)
jigsaw.open(options)
jigsaw.close()
jigsaw.submit(payload)
jigsaw.configure(options)
```
...
```ts
import { jigsaw } from "@jigsaw/browser"
...
- If no user is identified, Jigsaw uses anonymous visitor ID.
...
The host application should never crash because Jigsaw failed.
...
```tsx
import { JigsawProvider, useJigsaw } from "@jigsaw/react"

export function App() {
  return (
    <JigsawProvider projectKey="pk_live_abc123">
      <Dashboard />
    </JigsawProvider>
  )
}
...
```txt
[Jigsaw Feedback] Payment failed during checkout
```
...
- Jigsaw turns raw feedback into product insights

## 23. MVP User Stories

## 23.1 Developer Installs SDK

As a developer, I want to install Jigsaw with a few lines of code so that I can collect feedback without building my own system.
...
| Existing category | Limitation | Jigsaw advantage |
|---|---|---|
...
Jigsaw lets developers collect lightweight, contextual user feedback inside their apps and turn it into structured product signal.
```
...
Jigsaw can become the standard feedback infrastructure layer for software.
...
- Jigsaw becomes the system of record for user signal

The large vision:

> Jigsaw is the user signal layer for software.

## 40. Final Product Summary

Jigsaw helps developers collect better feedback by asking users lightweight questions at meaningful moments inside their applications.
